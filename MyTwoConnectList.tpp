...
template<typename T>
TwoConnectlist<T>::TwoConnectlist(TwoConnectlist&& other) noexcept : head_(std::move(other.head_)), tail_(other.tail_), size_(other.size_) {
    other.tail_ = nullptr;
    other.size_ = 0;
}
...
template<typename T>
TwoConnectlist<T>::~TwoConnectlist() {
...
template<typename T>
void TwoConnectlist<T>::push_back(const T& value) { push_back(T(value)); }

template<typename T>
void TwoConnectlist<T>::push_back(T&& value) {
...
template<typename T>
void TwoConnectlist<T>::insert(size_t index, const T& value) { insert(index, T(value)); }

template<typename T>
void TwoConnectlist<T>::insert(size_t index, T&& value) {
    if (index > size_) throw std::out_of_range("Out of range");
...
template<typename T>
void TwoConnectlist<T>::erase(size_t index) {
    if (index >= size_) throw std::out_of_range("Out of range");
    if (index == 0) {
...
template<typename T>
size_t TwoConnectlist<T>::size() const { return size_; }

template<typename T>
T& TwoConnectlist<T>::operator[](size_t index) { return get_node(index)->data_; }
